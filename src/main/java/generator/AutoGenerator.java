...
public class AutoGenerator extends AbstractGenerator {
    ...
    private Map <String, VelocityContext> analyzeData(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            ...
            //获取数据库类型，来生成mapper映射字段类型
            ctx.put("baseResultMapSql", IntfSqlMapUtil
                    .genBaseResultMapSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName(),
                            config.getDataSourceConfig().getDbType()));
            ctx.put("batchDeleteSql", IntfSqlMapUtil
                    .genBatchDeleteSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
            ctx.put("deleteSql",
                    IntfSqlMapUtil.genDeleteSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
            ...
            ctx.put("updateSql",
                    IntfSqlMapUtil.genUpdateSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
            ctx.put("getByIdSql",
                    IntfSqlMapUtil.genGetByIdSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
            ctx.put("getListSql",
                    IntfSqlMapUtil.genGetListSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
            ctx.put("getTotalSql",
                    IntfSqlMapUtil.genGetTotalSql(tableInfo.getFields(), tableInfo.getName(), tableInfo.getPrimaryKeyName(), tableInfo.getPropertyKeyName()));
        }
        return ctxData;
    }
    ...
    private void batchOutput(String entityName, VelocityContext context) {
        try {
            ...
            vm2File(context, dtoFile, template.getDto());
            //entity
            String entityFile = String.format((pathInfo.get(ConstVal.ENTITY_PATH) + ConstVal.ENTITY_NAME), entityName);
            vm2File(context, entityFile, template.getEntity());
            //entity
            String baseEntityFile =pathInfo.get(ConstVal.ENTITY_PATH) + File.separator + ConstVal.BASE_ENTITY + ConstVal.JAVA_SUFFIX;
            vm2File(context, baseEntityFile, template.getBaseEntity());
            //mapper
            String mapperFile = String
                    .format((pathInfo.get(ConstVal.MAPPER_PATH) + File.separator + tableInfo.getMapperName() + ConstVal.JAVA_SUFFIX), entityName);
            vm2File(context, mapperFile, template.getMapper());
            //baseMapper
            String baseMapperFile = pathInfo.get(ConstVal.MAPPER_PATH) + File.separator + ConstVal.BASE_MAPPER + ConstVal.JAVA_SUFFIX;
            vm2File(context, baseMapperFile, template.getBaseMapper());
            //mapper.xml
            String xmlFile = String.format((pathInfo.get(ConstVal.XML_PATH) + File.separator + tableInfo.getXmlName() + ConstVal.XML_SUFFIX), entityName);
            vm2File(context, xmlFile, template.getXml());
            //service
            String serviceFile = String
                    .format((pathInfo.get(ConstVal.SERIVCE_PATH) + File.separator + tableInfo.getServiceName() + ConstVal.JAVA_SUFFIX), entityName);
            vm2File(context, serviceFile, template.getService());
            //serviceImpl
            String implFile = String
                    .format((pathInfo.get(ConstVal.SERVICEIMPL_PATH) + File.separator + tableInfo.getServiceImplName() + ConstVal.JAVA_SUFFIX), entityName);
            vm2File(context, implFile, template.getServiceImpl());
            //controller
            String controllerFile = String
                    .format((pathInfo.get(ConstVal.CONTROLLER_PATH) + File.separator + tableInfo.getControllerName() + ConstVal.JAVA_SUFFIX), entityName);
            vm2File(context, controllerFile, template.getController());
            //baseController
            String baseControllerFile = pathInfo.get(ConstVal.CONTROLLER_PATH) + File.separator + ConstVal.BASE_CONTROLLER + ConstVal.JAVA_SUFFIX;
            vm2File(context, baseControllerFile, template.getBaseController());
            //baseService
            String baseServiceFile = pathInfo.get(ConstVal.SERIVCE_PATH) + File.separator + ConstVal.BASE_SERVICE + ConstVal.JAVA_SUFFIX;
            vm2File(context, baseServiceFile, template.getBaseService());
            //commonPage
            String commonPageFile = pathInfo.get(ConstVal.BASE_PATH) + File.separator + ConstVal.COMMON_PAGE + ConstVal.JAVA_SUFFIX;
            vm2File(context, commonPageFile, template.getCommonPage());
            //commonPage
            String returnResultFile = pathInfo.get(ConstVal.BASE_PATH) + File.separator + ConstVal.RETURNRESULT + ConstVal.JAVA_SUFFIX;
            vm2File(context, returnResultFile, template.getReturnResult());
     ...
     * @throws IOException
     */
    private void vm2File(VelocityContext context, String filePath, String templatePath) throws IOException {
    ...
    private void vmToFile(VelocityContext context, String templatePath, String outputFile) throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(outputFile); BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(fos, ConstVal.UTF8))) {
