package generator.config.builder;
...
import generator.config.rules.QuerySQL;
...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.BASE_CONTROLLER, joinPackage(config.getParent(), config.getController()));
        packageInfo.put(ConstVal.COMMON_PAGE, joinPackage(config.getParent(), config.getBase()));
        packageInfo.put(ConstVal.RETURNRESULT, joinPackage(config.getParent(), config.getBase()));
        ...
        if(StringUtils.isNotEmpty(template.getDto())){
        	pathInfo.put(ConstVal.DTO_PATH, joinPath(outputDir, packageInfo.get(ConstVal.DTO)));
        }
        ...
        if (StringUtils.isNotEmpty(template.getBaseController())) {
            pathInfo.put(ConstVal.CONTROLLER_PATH, joinPath(outputDir, packageInfo.get(ConstVal.BASE_CONTROLLER)));
        }
        if (StringUtils.isNotEmpty(template.getCommonPage())) {
            pathInfo.put(ConstVal.BASE_PATH, joinPath(outputDir, packageInfo.get(ConstVal.COMMON_PAGE)));
        }
        if (StringUtils.isNotEmpty(template.getReturnResult())) {
            pathInfo.put(ConstVal.BASE_PATH, joinPath(outputDir, packageInfo.get(ConstVal.RETURNRESULT)));
        }
    }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy, String[] tablePrefix) {
        for (TableInfo tableInfo : tableList) {
            ...
            if(StringUtils.isNotEmpty(tableInfo.getEntityName())){
            	tableInfo.setEntityLowerCaseName((new StringBuilder()).append(Character.toLowerCase(tableInfo.getEntityName().charAt(0))).append(tableInfo.getEntityName().substring(1)).toString());
            }
        }
        return tableList;
    }
        ...
        try {
            ...
            while (results.next()) {
                ...
                if (isId && !haveId) {
                    ...
                    tableInfo.setPrimaryKeyName(results.getString(querySQL.getFieldName()));
                ...
                if(field.isKeyFlag()){
                	tableInfo.setPropertyKeyName(field.getPropertyName());
                }
